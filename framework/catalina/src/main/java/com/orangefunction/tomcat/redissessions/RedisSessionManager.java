...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
	public static final String module = RedisSessionManager.class.getName();
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        for (Valve valve : getContext().getPipeline().getValves()) {
            if (valve instanceof RedisSessionHandlerValve) {
                ...
                Debug.logInfo("Attached to RedisSessionHandlerValve",module);
        ...
        try {
            ...
            Debug.logFatal( e,"Unable to load serializer",module);
         ...
         will serialize the session into Redis with 'set' instead of 'setnx'. */
            ...
            if (null != session) {
                try {
                    ...
                    Debug.logError(ex,"Error saving newly created session " ,module);
    ...
    public void add(Session session) {
        try {
            ...
            Debug.logWarning(ex,"Unable to add to session manager store " ,module);
    ...
    public Session findSession(String id) throws IOException {
        RedisSession session = null;
        Debug.logInfo("finding session id:" + id, module);
        if (null == id)
        {
            currentSessionIsPersisted.set(false);
            currentSession.set(null);
    ...
    public byte[] loadSessionDataFromRedis(String id) throws IOException {
        ...
        try {
            Debug.logVerbose("Attempting to load session " + id + " from Redis",module);
            ...
            if (data == null) {
                ...
                try {
                    ...
                    if (Debug.verboseOn()) {
                        Debug.logVerbose("Session Contents [" + id + "]:", module);
                        Enumeration en = session.getAttributeNames();
                        while (en.hasMoreElements()) {
                            Debug.logVerbose("  " + en.nextElement(),module);
                        }
                    }
                    save(session);
                    SessionSerializationMetadata updatedSerializationMetadata = new SessionSerializationMetadata();
                    byte[] sessionAttributesHash =serializer.attributesHashFrom(session);
                    updatedSerializationMetadata.setSessionAttributesHash(sessionAttributesHash);
                    data = serializer.serializeFrom(session, updatedSerializationMetadata);
                } catch (Exception ex) {
                    Debug.logFatal(ex,"Unable to deserialize into session", module);
                    throw new IOException("Unable to deserialize into session");
                }
                Debug.logVerbose("Session " + id + " not found in Redis",module);
            }
    ...
    public DeserializedSessionContainer sessionFromSerializedData(String id, byte[] data) throws IOException {
        Debug.logVerbose("Deserializing session " + id + " from Redis",module);

        if (Arrays.equals(NULL_SESSION, data)) {
            Debug.logError("Encountered serialized session " + id + " with data equal to NULL_SESSION. This is a bug.",module);
    ...
    protected boolean saveInternal(Jedis jedis, Session session, boolean forceSave) throws IOException {
        ...
        try
        {
            Debug.logVerbose("Saving session " + session + " into Redis",module);

            RedisSession redisSession = (RedisSession) session;

            if (Debug.verboseOn())
            {
                Debug.logVerbose("Session Contents [" + redisSession.getId() + "]:",module);
            ...
            Debug.logInfo("Saving session Id :" + redisSession.getId(), module);
            Boolean isCurrentSessionPersisted;
            SessionSerializationMetadata sessionSerializationMetadata = currentSessionSerializationMetadata.get();
            byte[] originalSessionAttributesHash = null;
            if(sessionSerializationMetadata != null){
            	 originalSessionAttributesHash = sessionSerializationMetadata.getSessionAttributesHash();
            }
                    ...
                    )
            {
                ...
                Debug.logVerbose("Save was determined to be necessary",module);
            ...
            {
                Debug.logVerbose("Save was determined to be unnecessary",module);
            }
            return false;
        }
        catch (IOException e) {
    ...
    public void remove(Session session, boolean update) {
        ...
        Debug.logVerbose("Removing session ID : " + session.getId(),module);
        ...
        if (redisSession != null) {
            try {
                if (redisSession.isValid()) {
                    Debug.logVerbose("Request with session completed, saving session " + redisSession.getId(),module);
                    save(redisSession, getAlwaysSaveAfterRequest());
                } else {
                    Debug.logVerbose("HTTP Session has been invalidated, removing :" + redisSession.getId(),module);
                    remove(redisSession);
                }
            } catch (Exception e) {
                Debug.logError(e,"Error storing/removing session", module);
            } finally {
                currentSession.remove();
                currentSessionId.remove();
                currentSessionIsPersisted.remove();
                Debug.logVerbose("Session removed from ThreadLocal :" + redisSession.getIdInternal(), module);
            }
        }
    }
    ...
    private void initializeDatabaseConnection() throws LifecycleException {
        try {
        	...
        	Debug.logError(e,"Error initialize Database Connection", module);
            throw new LifecycleException("Error connecting to Redis", e);
        }
    }

    private void initializeSerializer() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        Debug.logInfo("Attempting to use serializer :" + serializationStrategyClass, module);
