...
public class CatalinaContainer implements Container {
    ...
    private StandardContext configureContext(Engine engine, Host host, ComponentConfig.WebappInfo appInfo) throws ContainerException {
        ...
        //start modify by jimxu 2015-12-24 
        boolean isLoadRedisSession = false;
        //优先检查是否启用RedisSession
        boolean enabledRedisSession = UtilProperties.getPropertyValue("redis.properties", "redis.session.enabled").equals("Y");
        String redisSessionManagerClassName = UtilProperties.getPropertyValue("redis.properties", "redis.session.managerClassName");
        if (enabledRedisSession) {
	        Integer db =  UtilProperties.getPropertyAsInteger("redis.properties", "redis.session.db", 10);
	        if (db >= 0) {
	        	try {
	                Object redisSessionManager = Class.forName(redisSessionManagerClassName).newInstance();
	                IntrospectionUtils.setProperty(redisSessionManager, "host", 
	                		UtilProperties.getPropertyValue("redis.properties", "redis.session.host"));
	                IntrospectionUtils.setProperty(redisSessionManager, "port", 
	                		UtilProperties.getPropertyValue("redis.properties", "redis.session.port", "6379"));
	                IntrospectionUtils.setProperty(redisSessionManager, "password",  
	                		UtilProperties.getPropertyValue("redis.properties", "redis.session.password"));
	                IntrospectionUtils.setProperty(redisSessionManager, "database", db.toString());
	                IntrospectionUtils.setProperty(redisSessionManager, "maxInactiveInterval", "60");
	                sessionMgr = (Manager)redisSessionManager;
	                isLoadRedisSession = true;
	            } catch (Exception exc) {
	            	Debug.logError("Create Redis Session Manager Error: " + exc.getMessage(),module);
	            }
	        }
        }
        if (null != sessionMgr) {
	    	//添加RedisSessionHandlerValve
	    	Valve valve = null;
			try {
				//String valveClassName = UtilProperties.getPropertyValue("redis.properties", "redis.session.valveClassName");
				valve = (Valve)Class.forName("com.orangefunction.tomcat.redissessions.RedisSessionHandlerValve").newInstance();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				Debug.logError("Create RedisSessionHandlerValve Error: " + e.getMessage(),module);
			}
			if (valve != null) {
        ...
        if (!isLoadRedisSession) {
        	// 使用最新的redisSessionManager
        ...
        // configure persistent sessions
        // important: the call to context.setManager(...) must be done after Tomcat.initWebappDefaults(...)
        clusterProp = clusterConfig.get(engine.getName());
        if (clusterProp != null && contextIsDistributable) {
            String mgrClassName = ContainerConfig.getPropertyValue(clusterProp, "manager-class", "org.apache.catalina.ha.session.DeltaManager");
            try {
                sessionMgr = (Manager)Class.forName(mgrClassName).newInstance();
